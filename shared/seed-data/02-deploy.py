#!/usr/bin/env python3
"""
Seed Data Deployment Script (Step 2 of 2)

Deploys staged exports to iOS SeedData directory after review.
This is the second step in the 2-step export workflow.

Workflow:
  1. Stage (01-stage.py):  Database → staging/TIMESTAMP/ + HTML report
  2. Deploy (this script): Review → Deploy to iOS directory

Features:
- Lists available staged exports for deployment
- Creates automatic backup before deployment
- Deploys files from staging to iOS SeedData directory
- Tracks deployment history
- Provides rollback capability via backups

Usage:
    python 02-deploy.py --list-staged          # List available staged exports
    python 02-deploy.py TIMESTAMP              # Deploy specific staged export
    python 02-deploy.py --latest               # Deploy most recent staged export
    python 02-deploy.py --preview TIMESTAMP    # Preview deployment without deploying
    python 02-deploy.py --list-deployments     # Show deployment history
    python 02-deploy.py --confirm TIMESTAMP    # Deploy without confirmation prompt

Previous Step:
    python 01-stage.py                         # Create staged export
"""
...
from shared_utils import STAGING_ROOT, IOS_SEED_DATA_DIR, DEPLOYMENTS_DIR
...
class DeploymentManager:
    ...
    def __init__(
        ...
        ios_seed_data_dir: Path = IOS_SEED_DATA_DIR,
        ...
        self.ios_seed_data_dir = ios_seed_data_dir
    ...
    def print_staged(self):
        ...
        if not staged:
            ...
            print(f"\nRun: python scripts/01-stage.py")
            return
        ...
        print(f"To deploy: python scripts/02-deploy.py {staged[0]['timestamp']}")
    ...
    def get_staged_export(self, timestamp: str) -> Optional[Path]:
        ...
        Args:
            timestamp: Export timestamp (or "latest")
    ...
    def deploy(
        ...
        """
        print(f"\n{Fore.CYAN}{'='*60}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}DEPLOYMENT - Step 2 of 2{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'='*60}{Style.RESET_ALL}\n")
        ...
        # Clean slate: Remove all existing seeddata_* files before deployment
        print(f"\n{Fore.YELLOW}Removing old seeddata files...{Style.RESET_ALL}")
        existing_files = list(self.ios_seed_data_dir.glob("seeddata_*.json"))
        removed_count = 0
        for old_file in existing_files:
            try:
                ...
                print(f"{Fore.YELLOW}Warning: Could not remove {old_file.name}: {str(e)}{Style.RESET_ALL}")

        if removed_count > 0:
            print(f"{Fore.GREEN}✓ Removed {removed_count} old files{Style.RESET_ALL}")
        else:
            print(f"{Fore.GREEN}✓ No old files to remove{Style.RESET_ALL}")
    ...
    def preview_deploy(self, timestamp: str):
        ...
        print(f"\nTo deploy: python scripts/02-deploy.py {staged_path.name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Deploy staged export to iOS SeedData (Step 2 of 2)"
    )
    parser.add_argument(
        'timestamp',
        nargs='?',
        help="Staged export timestamp to deploy (e.g., 2025-12-05_180000)"
    )
    parser.add_argument(
        '--list-staged',
        action='store_true',
        help="List all available staged exports"
    )
    parser.add_argument(
        '--latest',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=str,
        metavar='TIMESTAMP',
        help="Preview deployment without deploying"
    )
    parser.add_argument(
        '--list-deployments',
        action='store_true',
    ...
    parser.add_argument(
        '--no-backup',
        action='store_true',
        help="Skip creating backup before deployment (not recommended)"
    )
    ...
    if args.latest:
        ...
        print(f"{Fore.RED}Error: No timestamp specified{Style.RESET_ALL}\n")
        parser.print_help()
        sys.exit(1)
